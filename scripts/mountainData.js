...
function mountainsDropDownList() {
  for (let i = 0; i < mountainsArray.length; i++) {
    let mountName = mountainsArray[i].name;
    const options = document.createElement("option");
    options.value = mountName.toLowerCase();
    options.innerText = mountName;
    select.appendChild(options);
  }
}
mountainsDropDownList();
...
function tableBuilder(tbody, array) {
  ...
  cell2.innerText = array.name + " " + array.elevation + ", " + array.effort;
  let cell3 = row.insertCell(1);
  cell3.innerText = array.desc;
  let cell4 = row.insertCell(2)
  cell4.innerHTML = `<img src='images/${array.img}'>`
}
function selectedMount() {
    clearTableList()
  let mount = mountainsArray.find(function (mnt) {
    return mnt.name.toLowerCase() == select.value;
  });
  tableBuilder(outputTable, mount);
}
